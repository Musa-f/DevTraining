...
function ventilateur(a,b) {
	let result = 0;
	for(let i=a; i<a; i++) { //problème car i=a dont ne peut pas être inférieur
		result += b;
	}
	return result;
}
...
function minecraft(a) {
	let result = 1;
	for(let i=0; i<a; i++) { // donc si a égal à 0 ou 1 la boucle est ignorée
		result *= a;
	}
	return result;
}
...
function nenio(a) {
	let result = Math.random(); // entre 0 et 1
	result = Math.floor(result); // valeur inférieure donc 0
	return result;
}
...
function spiderman(a) {
	let result = 1;
	for(let i=0; i<a; i++) {
		result = bouteilleDEau(result); //on a -1 donc si on repète i c'est négatif
	}
	return result;
}
